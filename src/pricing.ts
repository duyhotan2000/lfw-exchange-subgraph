...
import { BNBContract } from "../generated/Factory/BNBContract";
...
let BNB_BUSD_PAIR = "0x1B96B92314C44b159149f7E0303511fB2Fc4774f";


export function getBnbPriceInUSD(): BigDecimal {
  const bnbContract = BNBContract.bind(Address.fromString(BNB_BUSD_PAIR));
  log.info("BNBCONTRACT: {}", [bnbContract._address.toHex()]);
  const reserves = bnbContract.try_getReserves();
  if(reserves.reverted) {
    return ZERO_BD;
  }
  const BNBPrice = reserves.value.value1.divDecimal(reserves.value.value0.toBigDecimal());
  log.info("BNBPrice: {}", [BNBPrice.toString()]);
  return BNBPrice;
