...
import { updatePairDayData, updateTokenDayData, updatePairHourData, updateLinkedFinanceDayData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  let linkedWorld = LinkedFinanceWorldFactory.load(FACTORY_ADDRESS)!;

  // reset factory liquidity by subtracting only tracked liquidity
  linkedWorld.totalLiquidityBNB = linkedWorld.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  linkedWorld.totalLiquidityBNB = linkedWorld.totalLiquidityBNB.plus(trackedLiquidityBNB);
  linkedWorld.totalLiquidityUSD = linkedWorld.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  linkedWorld.totalTransactions = linkedWorld.totalTransactions.plus(ONE_BI);
  ...
  updateLinkedFinanceDayData(event);
...
export function handleSwap(event: Swap): void {
  ...
  let linkedWorld = LinkedFinanceWorldFactory.load(FACTORY_ADDRESS)!;
  linkedWorld.totalVolumeUSD = linkedWorld.totalVolumeUSD.plus(trackedAmountUSD);
  linkedWorld.totalVolumeBNB = linkedWorld.totalVolumeBNB.plus(trackedAmountBNB);
  linkedWorld.untrackedVolumeUSD = linkedWorld.untrackedVolumeUSD.plus(derivedAmountUSD);
  linkedWorld.totalTransactions = linkedWorld.totalTransactions.plus(ONE_BI);
  ...
  let pancakeDayData = updateLinkedFinanceDayData(event);
